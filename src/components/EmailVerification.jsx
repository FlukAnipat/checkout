...
export default function EmailVerification({ 
  ...
  const sendOTP = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (response.otp) {
          console.log(`📧 Email OTP for ${email}: ${response.otp}`)
        }
      }
    } catch (err) {
      setError(err.error || 'Failed to send email OTP')
    } finally {
      setIsSending(false)
    }
  }
  ...
  const handleVerifyOTP = async (otpValue) => {
    ...
    try {
      const response = await otpService.verifyEmailOTP(email, otpValue)
  ...
  const resendOTP = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // For development, show the OTP
        if (response.otp) {
          console.log(`📧 Resent Email OTP for ${email}: ${response.otp}`)
        }
      }
    } catch (err) {
      setError(err.error || 'Failed to resend email OTP')
    } finally {
      setIsSending(false)
    }
  }
      ...
      <div className="text-center space-y-4">
        {!canResend ? (
          ...
          <button
            onClick={resendOTP}
            disabled={isSending}
            className="flex items-center justify-center gap-2 px-4 py-2 text-sm text-blue-600 hover:text-blue-700 font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isSending ? (
              <>
